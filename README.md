# Issue H-1: Adversary can break deposit queue and cause loss of funds 
...
# Issue H-2: When rolling over, user will lose his winnings from previous epoch 
...
# Issue H-4: Earlier users in rollover queue can grief later users 
...
# Issue H-5: Funds can be stolen because of incorrect update to `ownerToRollOverQueueIndex` for existing rollovers 
...
# Issue M-1: Inconsistent use of epochBegin could lock user funds 
...
# Issue M-2: Null epochs will freeze rollovers 
...
# Issue M-3: VaultFactoryV2#changeTreasury misconfigures the vault 
...
# Issue M-4: Arbitrum sequencer downtime lasting before and beyond epoch expiry prevents triggering depeg 
...
# Issue M-5: `Carousel.mintRollovers` potentially mints `0` shares and can grief rollover queue 
...
# Issue M-6: [M-2] Vault Factory ownership can be changed immediately and bypass timelock delay 
...
# Issue M-7: mintRollovers should require entitledShares >= relayerFee 
...
# Issue M-8: changeTreasury() Lack of check and remove old 
...
# Issue M-9: User deposit may never be entertained from deposit queue 
...
# Issue M-10: Malicious user can make rolloverQueue never get processed 
...
# Issue M-11: Stuck emissions for nullified epochs 
...
# Issue M-12: Controller doesn't send treasury funds to the vault's treasury address 
...
# Issue M-13: ControllerPeggedAssetV2: `triggerEndEpoch` function can be called even if epoch is null epoch leading to loss of funds 
...
# Issue M-14: ControllerPeggedAssetV2: outdated price may be used which can lead to wrong depeg events 
